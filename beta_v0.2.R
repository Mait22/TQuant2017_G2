...
dataGen = function(n, generator = 1,plot=F) {
  ...
  if(generator == 1){
    ...
    x2 = rnorm(n,20,8)
    x3 = rnorm(n,4,70)
    x4 = rep(runif(4,-10,40),trunc(n/4))
    y = gen4(x1,x2,x3,x4)
  }
...
par(mfrow=c(1,2))
dataGen(100,plot=T,generator = 4)
dataGen(1000,plot=T,generator = 4)
par(mfrow=c(1,1))
...
jaVal("y~x1+I(x1^2)+I(x1^3)+I(x1^4)+I(x1^5)+I(x1^6)+I(x1^7)+I(x1^8) + x1*x2 + I(x2^2) + I(x2^3)",
      dataGen(n=100,4),folds = 5,repetitions = 10)

jaVal("y~x1+I(x1^2)+I(x1^3)",
      dataGen(n=100,4),folds = 5,repetitions = 10)
